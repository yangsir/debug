...
elseif($method=='batch')
{
	...
	elseif($ckdel=='2' || $ckdel=='3'){
		$sql = "SELECT count(*) FROM {pre}vod where 1=1 ".$where;
		$nums = $db->getOne($sql);
		$pagecount=ceil($nums/$pagesize);
		
	    $sql = "SELECT d_id,d_name,d_playfrom,d_playurl,d_playserver,d_playnote,d_downfrom,d_downurl,d_downserver,d_downnote FROM {pre}vod where 1=1 ".$where . " ORDER BY d_id desc limit ".($pagesize * ($pagecount-1)) .",".$pagesize;
		...
		if($nums==0){
			...
			while ($row = $db ->fetch_array($rs))
			{
				...
				$d_downserver = $row["d_downserver"];
				$d_downurl = $row["d_downurl"];
				$d_downnote = $row['d_downnote'];
				...
				$downurlarr = explode("$$$",$d_downurl);
				$downnotearr = explode("$$$",$d_downnote);
				...
				$rc=false;
				for ($i=0;$i<count($downfromarr);$i++){
					if($downfromarr[$i]==$down){
						...
						$new_downfrom .= $downfromarr[$i];
						$new_downserver .= $downserverarr[$i];
						$new_downnote .= $downnotearr[$i];
						$new_downurl .= str_replace("'","''",$downurlarr[$i]);
						$rc=true;
					}
				}
				
				$sql = "UPDATE {pre}vod set ";
				if($ckdel=='2'){
					$sql .= "d_playfrom='".$new_playfrom."',d_playserver='".$new_playserver."',d_playnote='".$new_playnote."',d_playurl='".$new_playurl."'";
				}
				if($ckdel=='3'){
					$sql .= "d_downfrom='".$new_downfrom."',d_downserver='".$new_downserver."',d_downnote='".$new_downnote."',d_downurl='".$new_downurl."'";
				}
				$sql .= " where d_id='".$row["d_id"]."'";
				$db->query($sql);
