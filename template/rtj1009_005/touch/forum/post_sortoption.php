...
<!--{else}-->
	<ul class="cl">
	<!--{if $_G['forum']['threadsorts']['expiration'][$_G['forum_selectsortid']]}-->
		<li class="ren_flxx_li">
			<div class="ren_flxx_lx">{lang threadtype_expiration}</div>
	...
	<!--{loop $_G['forum_optionlist'] $optionid $option}-->
		<li class="ren_flxx_li<!--{if $option['type'] == 'number' || $option['type'] == 'text' || $option['type'] == 'select' || $option['type'] == 'range'}--> ren_flxx_de<!--{elseif $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'textarea'}--> ren_flxx_he<!--{elseif $option['type'] == 'image'}--> ren_flxx_me<!--{/if}-->">
			<div class="ren_flxx_lx">$option[title]<!--{if $option['required']}--><span class="rq"> *</span><!--{/if}--></div>
			<div class="ren_flxx_lxnr">
				<div id="select_$option[identifier]" class="ren_select">
				<!--{if in_array($option['type'], array('number', 'text', 'email', 'calendar', 'image', 'url', 'range', 'upload', 'range'))}-->
					...
					<!--{elseif $option['type'] == 'image'}-->
                        ...
                        <div id="sortaid_{$option[identifier]}" class="ren_flpost_img cl">
                            <a href="javascript:;" class="ren_bl_fjy">
                                <input type="file" name="sortaid_{$option[identifier]}" id="sortaid_{$option[identifier]}"/>
                                <em>选择图片</em>
                            </a>
                        </div>
						...
						<!--{/if}-->    
						<div id="sortattach_image_{$option[identifier]}" class="ren_flpost_pic">
						<!--{if $option['value']['url']}-->
							<a href="$option[value][url]"><img class="spimg" src="$option[value][url]" alt="" /></a>
						<!--{/if}-->
						</div>
<script type="text/javascript">
	$(document).on('change', '#sortaid_{$option[identifier]}', function() {
			...
			uploadsuccess = function(data) {
				...
				if(dataarr[0] == 'DISCUZUPLOAD' && dataarr[2] == 0) {
					popup.close();
						...
						$('#sortattach_image_{$option[identifier]}').html('<a href="{$_G[setting][attachurl]}forum/'+dataarr[5]+'" target="_blank"><img src="{$_G[setting][attachurl]}forum/'+dataarr[5]+'" /></a>');
				} else {
					var sizelimit = '';
					if(dataarr[7] == 'ban') {
			...
			if(typeof FileReader != 'undefined' && this.files[0]) {//note 支持html5上传新特性
				...
				$.ajaxfileupload({
					...
					<!--{else}-->
						<input type="text" name="typeoption[{$option[identifier]}]" id="typeoption_$option[identifier]" class="px" tabindex="1" size="$option[inputsize]" onBlur="checkoption('$option[identifier]', '$option[required]', '$option[type]'{if $option[maxnum]}, '$option[maxnum]'{else}, '0'{/if}{if $option[minnum]}, '$option[minnum]'{else}, '0'{/if}{if $option[maxlength]}, '$option[maxlength]'{/if})" placeholder="<!--{if $option['maxnum'] || $option['minnum'] || $option['maxlength'] || $option['unchangeable'] || $option[description]}-->
					<!--{if $option['maxnum']}-->
					...
					<!--{if $option[description]}-->
						$option[description]
					<!--{/if}-->
				<!--{/if}-->" value="{if $_G['tid']}$option[value]{else}{if $member_profile[$option['profile']]}$member_profile[$option['profile']]{else}$option['defaultvalue']{/if}{/if}" $option[unchangeable] />
					<!--{/if}-->
	...
	<!--{/loop}-->
    <li class="ren_flxx_he"><div class="ren_tie_ksf">内容</div></li>
	</ul>
<!--{/if}-->
