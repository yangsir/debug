...
<header class="header">
	<div class="nav">
		<div class="header-left">
            <a href="<!--{if $_GET[action] == 'newthread'}-->forum.php?mod=forumdisplay&fid=$_G[fid]&page=$_GET[page]<!--{else}-->forum.php?mod=redirect&goto=findpost&ptid=$_G[tid]&pid=$pid<!--{/if}-->"><i class="icon icon-back"></i></a>
        </div>
		<div class="header-title"><!--{if $_GET[action] == 'edit'}-->{lang edit}<!--{else}-->{lang send_threads}<!--{/if}--></div>
	</div>
</header>
...
<div class="wp">
	<div class="post_from">
		<ul class="cl">
			<li class="bl_line">
			<!--{if $_GET['action'] != 'reply'}-->
			<input type="text" tabindex="1" class="px" id="needsubject" size="30" autocomplete="off" value="$postinfo[subject]" name="subject" placeholder="{$language['please_input']}{lang thread_subject}" fwin="login">
			<!--{else}-->
				RE: $thread['subject']
			...
			<textarea class="pt" id="needmessage" tabindex="3" autocomplete="off" id="{$editorid}_textarea" name="$editor[textarea]" cols="80" rows="2"  placeholder="{$language['please_input']}{lang thread_content}" fwin="reply">$postinfo[message]</textarea>
			</li>
			<li style="padding:0px;">
				<a href="javascript:;" class="post_photo z">
		...
		<div class="post_imgtip cl">
			<p>{lang attachment_allow_exts}: <span>jpg, jpeg, gif, png</span></p>
			<p>{lang attachment_size}: <span><!--{if $_G['group']['maxattachsize']}-->{lang lower_than} $maxattachsize_mb <!--{else}-->{lang size_no_limit}<!--{/if}--></span></p>
		</div>
	...
	$(document).on('change', '#filedata', function() {
			...
			uploadsuccess = function(data) {
				...
				if(dataarr[0] == 'DISCUZUPLOAD' && dataarr[2] == 0) {
					popup.close();
					$('#imglist').append('<li><span aid="'+dataarr[3]+'" class="del"><a href="javascript:;"><i class="icon_del"></i></a></span><span class="p_img"><a href="javascript:;"><img style="height:54px;width:54px;" id="aimg_'+dataarr[3]+'" title="'+dataarr[6]+'" src="{$_G[setting][attachurl]}forum/'+dataarr[5]+'" /></a></span><input type="hidden" name="attachnew['+dataarr[3]+'][description]" /></li>');
				} else {
					var sizelimit = '';
					if(dataarr[7] == 'ban') {
