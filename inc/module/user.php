...
if($method=='iframe')
{
    ...
    if ($logged){
        $row = $db->getRow("SELECT u_id,u_name,u_qq,u_email,u_phone,u_regtime,u_status,u_points,u_extend,u_loginnum, u_logintime,u_loginip,u_flag,u_start,u_end,u_group FROM {pre}user where u_id=".$_SESSION["userid"] );
        if($row){
...
elseif($method=='check')
{
	$u_name = be("post","u_name"); $u_name = chkSql($u_name);
...
elseif($method=='regcheck')
{
	...
	$status="true";
	$s=$tpl->P['s']; $s=chkSql($s);
	$t=$tpl->P['t']; $t=chkSql($t);
	switch($t)
	{
...
elseif($method=='regsave')
{
	...
	if (!$row){
		 $colarr = array("u_name", "u_password","u_qq","u_email","u_regtime","u_status","u_points","u_group","u_phone","u_question","u_answer",'u_qid','u_logintime','u_loginnum','u_extend','u_loginip','u_random','u_fav','u_plays','u_downs','u_start','u_end');
		 $valarr = array($u_name,$u_password1,$u_qq,$u_email,time(),$MAC['user']['regstate'],$MAC['user']['regpoint'],$MAC['user']['reggroup'],$u_phone,$u_question,$u_answer,'',0,0,0,0,'','','','',0,0);
		 
		 
		 $db->Add  ("{pre}user",$colarr,$valarr);
...
elseif($method=='tg')
{
	if ($MAC['user']['popularizestate'] == 0){ redirect ("/");exit; }
	
	$userid = intval($tpl->P['uid']);
	
	if($userid>0){
		$ip = ip2long(getIP());
		$ly=  getReferer(); StopAttack('referer',$ly,$getfilter); $ly=chkSql($ly);
...
elseif($method=='save')
{
	...
	$u_qq= be("post","u_qq"); $u_qq = chkSql($u_qq);
	$u_email = be("post","u_email"); $u_email = chkSql($u_email);
...
elseif($method=='del')
{
	...
	if(!empty($clear)){
		...
		$ids = be("arr","d_id");  $ids = chkSql($ids);
		$ids = ",".$ids.",";
		$data = $user['u_'.$flag];
	...
	elseif ($ref=="qqlogged"){
		...
		if($qc->checkLogin()){
			...
			if ($rscount == 0){
				...
				$db->Add( "{pre}user",array("u_name","u_qid", "u_password","u_qq","u_email","u_regtime","u_status","u_points","u_group","u_phone","u_question","u_answer",'u_logintime','u_loginnum','u_extend','u_loginip','u_random','u_fav','u_plays','u_downs','u_start','u_end'),array($nickname,$qqid,md5(""),"","",time(),$MAC['user']['regstate'],$MAC['user']['regpoint'],$MAC['user']['reggroup'],"","","",0,0,0,0,'','','','',0,0));
			}
