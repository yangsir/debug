...
{
	if($GLOBALS['MAC']['db']['type']=='mysql'){
		$GLOBALS['db'] = new AppMysql($GLOBALS['MAC']['db']['server'],$GLOBALS['MAC']['db']['user'],$GLOBALS['MAC']['db']['pass'],$GLOBALS['MAC']['db']['name']);
	}
	else{
		$GLOBALS['db'] = new AppMysqli($GLOBALS['MAC']['db']['server'],$GLOBALS['MAC']['db']['user'],$GLOBALS['MAC']['db']['pass'],$GLOBALS['MAC']['db']['name']);
	}
}
function dispseObj()
{
...
function chkCache($cp,$cn)
{
	...
	if($GLOBALS['MAC']['app']['cachetype']==0){
		$cf = MAC_ROOT.'/cache/'.$cp.'/'.$GLOBALS['MAC']['app']['cacheid'].MAC_MOB.'_'.$cn.'.inc';
...
function getUserPopedom($id,$flag)
{
	...
	$groupcache = $GLOBALS['MAC_CACHE']['usergroup'];
	$curgroup = $groupcache[$usergroup];
	if(!is_array($curgroup)) { $curgroup = array("ug_popvalue"=>0);  $ug_popvalue1=$curgroup["ug_popvalue"]; }
