...
class AppTpl
{
    ...
    function getLink($f,$l,$t,$d,$pg='')
    {
	    ...
	    switch($l)
	    {
	    	...
	    	case 'search':
	    		...
	    		if($valarr['wd']==''){ $str = str_replace('-wd-{wd}','',$str); }
				break;
	    	case 'list':
	    		$strpg='';
    ...
    function mark_sql()
    {
		...
		if(empty($lp['table'])) { $lp['table'] = 'vod'; }
        if(empty($lp['type'])) { $lp['type'] = 'all'; }
		...
		switch($this->markname)
		{
			...
			case 'vod':
		        ...
		        if(!empty($lp['similar']) && is_array($this->P['similar'])){
		        	switch($lp['similar'])
		        	{
		        		case "name": $cola='name';break;
		        		case "tag": $cola= 'tag';break;
		        		case "directed": $cola = 'directed';break;
		        		default:$cola='starring';break;
		        	}
		        	...
		        	if(count($tmp)>0){
			        	...
			        	if($rc){ $where .= " AND (" . $where2 . ") and d_id <> ". intval($this->D['d_id'])." ";  }
		        	}
		        	unset($tmp);
		        }
				break;
			...
			case 'art':
		        ...
		        if(!empty($lp['similar']) && is_array($this->P['similar'])){
		        	...
		        	if(count($tmp)>0){
			        	...
			        	if($rc){ $where .= " AND (" . $where2 . ") and a_id <> ". intval($this->D['a_id'])." ";  }
		        	}
		        	unset($tmp);
		        }
				break;
            ...
            case "vod":
                ...
                switch($m2)
                {
                    ...
                    case "picslide":
                        ...
                        break;
                    case "letter": $val=getTextt($m3, $mrs["d_letter"]);break;
                    case "letterlink": $val=getKeysLink($mrs["d_letter"], "letter");break;
                    ...
                    case "playlinks":
                    	...
                    	$cc = count($arr);
                    	if(empty($mrs['d_playfrom']) || $cc==0) { break; }
                    ...
                    case "downlinks":
                    	...
                    	$cc = count($arr);
                    	if(empty($mrs['d_downfrom']) || $cc==0) { break; }
            ...
            case "art":
                ...
                switch($m2)
				{
                    ...
                    case "colorname":
                    	...
                    	break;
                    case "letter": $val=getTextt($m3, $mrs["a_letter"]);break;
                    case "letterlink": $val=getKeysLink($mrs["a_letter"], "letter",'art');break;
                    ...
                    case "tag" : $val=getTextt($m3, $mrs["a_tag"],'art');break;
                    case "taglink": $val=getKeysLink($mrs["a_tag"], "tag",'art');break;
                    case "content":
                    	$val = $mrs["a_content"];
                    ...
                    case "up": $val=$mrs["a_up"];break;
                    case "down": $val=$mrs["a_down"];break;
    ...
    function playdownlist($flag)
    {
        ...
        for($i=0;$i<count($matches1[0]);$i++)
		{
            ...
            if($this->L['type'] =='mode2'){
            	...
            	if (count($matches3[0])>0 && $urlarrlen >=0 && $fromarrlen==$urlarrlen){
            		$markorder = $matches3[1][0];
	                $urlarr1 = explode("#",$urlarr[0]);
	                $urlarr1len = count($urlarr1);
	                
	                for ($j=0;$j<$urlarr1len;$j++){
	                	$markslist2 = "";
						$flagname="";
	                	for ($k=0;$k<$fromarrlen;$k++){
	                		$num = $num + 1;
						 	$url = $urlarr[$k];
						 	$urlpath = "no";
							$urlfrom = $fromarr[$k];
						 	...
						 	if ($j < count($urlarr1)){
								...
								if($flagname==''){ $flagname = $urlname; }
		                    }
							if($urlpath=='no'){ $urlfrom = $fromarr[$k].'no'; $urlname='暂缺'; }
							
							$url = str_replace(array('{src}','{num}'),array($k+1,$j+1),$lnk);
							$markstr = str_replace('[url:num]', $m+1 ,$matches3[2][0]);
							$markstr = str_replace(array('[url:from]','[url:name]','[url:path]','[url:link]'), array($urlfrom,$urlname,$urlpath,$url),$markstr);
							if($this->L['order']=="desc"){
								$markslist2 = $markstr . $markslist2;
							}
							else {
								$markslist2 .= $markstr; 
							}
                        ...
                        $markhtml = str_replace($matches3[0][0] ,$markslist2,$this->markdes);
                        $markhtml = str_replace(array('['.$flag.':urlcount]','['.$flag.':count]','['.$flag.':num]','['.$flag.':name]'), array($urlarr1len,$fromarrlen,($j+1),$flagname) ,$markhtml);
                        
                        $markslist1[$j] = $markhtml;
	                }
	                if(strpos($markorder,"desc")){
                    	krsort($markslist1);
                    }
            	}
            }
    ...
    function getUrlName($flag)
    {
		...
		$this->H = str_replace("[vod:".$flag."src]",$this->P['src'],$this->H);
		$this->H = str_replace("[vod:".$flag."num]",$this->P['num'],$this->H);
		
		$this->H = str_replace("[vod:".$flag."from]",$curfrom,$this->H);
		$this->H = str_replace("[vod:".$flag."show]",$show,$this->H);
    ...
    function loadtype($flag)
    {
        ...
        if ($flag == 'art'){
			...
			$col=array('{page:linktype}','{page:linkyear}','{page:linkletter}','{page:linkarea}','{page:linklang}','{page:linkclass}');
			$this->H = str_replace($col,$linkbytime,$this->H);
    ...
    function loadvod($flag)
    {
        if($this->P['make']!=true && chkCache($this->P['cp'],$this->P['cn'])){
            ...
            $this->H = loadFile($tp);
            $this->C["sitetid"] = $this->D['d_type'];
            $this->C["siteid"] = $this->D['d_id'];
            $this->P['vodtypeid'] = $this->T['t_id'];
            $this->P['vodtypepid'] = $this->T['t_pid'];
            $this->P['similar'] = array('name'=>$this->D['d_name'],'starring'=>$this->D['d_starring'],'tag'=>$this->D['d_tag'],'directed'=>$this->D['d_directed']);
            $GLOBALS['MAC_CACHE']['vodtype'][$this->T['t_id']]['similar']='no';
            if($this->P['make']==true){
            	if(strpos($this->H,'similar=')>0){
    				$GLOBALS['MAC_CACHE']['vodtype'][$this->T['t_id']]['similar'] = 'ok';
    			}
            }
