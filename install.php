...
<br /><br />
	...
	<strong>版权所有 (c) 2008-2017，苹果MacCMS,
	  保留所有权利</strong>。 
	</div>
...
function checkdb(){
    	...
    	$.ajax({cache: false, dataType: 'html', type: 'GET', url: 'install.php?action=ckdb&server='+server+'&db='+dbname+'&type='+dbtype+'&id='+id+'&pwd='+encodeURIComponent(pwd),
...
<div id="form_items_3" ><br /><div class="desc"><b>填写网站配置信息</b></div>
	...
	if(function_exists('mysql_connect')){
		echo '<option value="mysql">mysql连接</option>';
	}
	if(function_exists('mysqli_connect')){
		echo '<option value="mysqli">mysql-i连接</option>';
	}
	?>
	</select></td>
	<td>网站使用数据库的类型</td>
...
EOT;
	...
	if($db_type=='mysql'){
		...
		if(!$lnk){
			...
			if(!@mysql_select_db($db_name,$lnk)){
				...
				$dbck=true;
				$db = new AppMysql($db_server,$db_user,$db_pass,$db_name);
			} 
		}
	}
	else{
		$lnk=@mysqli_connect($db_server,$db_user,$db_pass);
		if(!$lnk){
			echo '<script type="text/javascript">showmessage(\'数据库设置出错：mysql请检查数据库连接信息... \');</script>';
		}
		else{
			if(!@mysqli_select_db($lnk,$db_name)){
				...
				$dbck=true;
				$db = new AppMysqli($db_server,$db_user,$db_pass,$db_name);
			} 
		}
	}
  ...
  `a_content` mediumtext NOT NULL DEFAULT '',
  PRIMARY KEY (`a_id`),
  KEY `a_type` (`a_type`),
  KEY `a_level` (`a_level`),
  KEY `a_hits` (`a_hits`),
  ...
  `t_score` decimal(3,1) NOT NULL DEFAULT '0',
  ...
  `u_fav` text NOT NULL DEFAULT '',
  `u_plays` text NOT NULL DEFAULT '',
  `u_downs` text NOT NULL DEFAULT '',
